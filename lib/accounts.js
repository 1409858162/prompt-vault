...
export async function createAccount({
  ...
  const insert = async (dbConn) => {
    ...
    const [created] = await dbConn.execute(
  ...
  // Transaction so the username-uniqueness check + insert is atomic. When a
  // caller supplies `conn`, it owns the outer transaction and can atomically
  // pair account creation with invite consumption.
...
export async function verifyPasswordIncludingRevoked(username, password) {
  const a = await findByUsername(username);
  if (!a) {
    await bcrypt.compare(password || 'x', '$2a$10$abcdefghijklmnopqrstuv1234567890ABCDEFGHIJKLMNOPQRSTUV');
    return null;
  ...
  const result = await execute(
    `UPDATE accounts
     SET password_hash = ?,
         updated_at = ?
     WHERE id = ?`,
    [password_hash, toMysqlDt(new Date()), id]
  );
  if (result.affectedRows === 0) return null;
  return await findById(id);
}
...
export async function applyInviteRedemption(id, {
  promoted_from_code,
