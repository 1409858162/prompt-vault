...
export async function findByPlaintext(plaintext) {
  const fingerprint = crypto.createHash('sha256').update(String(plaintext || '')).digest('hex');
  ...
  if (direct) {
    try {
      return await bcrypt.compare(plaintext, direct.code_hash) ? rowToCode(direct) : null;
    } catch {
      return null;
    }
  }
  if (![32, 50].includes(String(plaintext || '').length)) return null;

  // Legacy migrated rows may not have a fingerprint because only the bcrypt
  // hash existed. Keep a narrow fallback for those old codes without making
  // every normal lookup scan the whole invite table. Some pre-fingerprint
  // batches were already 32 chars, while the oldest codes were 50 chars.
  ...
  for (const row of rows) {
    let matched = false;
    try {
      matched = await bcrypt.compare(plaintext, row.code_hash);
    } catch {}
    if (matched) {
      try {
        await execute(
          `UPDATE codes
           ...
           WHERE id = ?
             AND code_fingerprint IS NULL`,
          [fingerprint, toMysqlDt(new Date()), row.id]
        );
      } catch (err) {
        if (err?.code !== 'ER_DUP_ENTRY') {
          console.warn('[codes] failed to backfill invite fingerprint:', err?.message || err);
        }
      }
      return rowToCode({ ...row, code_fingerprint: fingerprint });
    }
  }
  return null;
}
...
export async function add({ label = '', note = '', membership_years = DEFAULT_MEMBERSHIP_YEARS } = {}) {
  ...
  const effectiveYears = years > 0 ? years : DEFAULT_MEMBERSHIP_YEARS;

  for (let attempt = 0; attempt < 5; attempt++) {
    const plaintext = generateCode();
    const hash = await bcrypt.hash(plaintext, 10);
    const code_fingerprint = crypto.createHash('sha256').update(plaintext).digest('hex');
    const id = 'c_' + crypto.randomBytes(10).toString('hex');

    try {
      await execute(
        `INSERT INTO codes
          (id, code_hash, code_fingerprint, label, note, revoked, created_at, last_used_at,
...
export async function consumeForPromotion(codeId, accountId, conn = null) {
  const now = toMysqlDt(new Date());
  const sql = `UPDATE codes
     SET login_disabled = 1,
         consumed_for_account = ?,
         consumed_at = ?,
         updated_at = ?
     WHERE id = ?
       AND revoked = 0
       AND login_disabled = 0
       AND consumed_for_account IS NULL`;
  const params = [accountId, now, now, codeId];
  const result = conn
    ? (await conn.execute(sql, params))[0]
