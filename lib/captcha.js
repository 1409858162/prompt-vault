...
if (!SECRET) {
  throw new Error('CAPTCHA_SECRET or JWT_SECRET is required when NODE_ENV=production');
}
...
export async function issueCaptcha() {
  ...
  const sig = crypto.createHmac('sha256', SECRET).update(`${id}:${answer}:${exp}`).digest('base64url');
  ...
  const svgB64 = b64url(Buffer.from(svg, 'utf8'));
