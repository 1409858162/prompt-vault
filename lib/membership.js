...
export function resolveAccountMembership(account, codeEntry) {
  if (!account) return getMembershipStatus(null);
  if (account.revoked) {
    return getMembershipStatus({
      revoked: true,
      membership_years: account.membership_years ?? null,
      activated_at: account.activated_at ?? null,
      expires_at: account.expires_at ?? null,
    }, { revoked: true });
  }

  const hasAccountMembership =
    account.membership_years != null ||
    !!account.activated_at ||
    !!account.expires_at;

  // Plain self-registered accounts have no invite source and no entitlement.
  // Do not let the generic "legacy permanent code" rule treat this empty
  // account-shaped object as an old invite code.
  if (!hasAccountMembership && !codeEntry) {
    return {
      is_member: false,
      is_permanent: false,
      expires_at: null,
  ...
  return getMembershipStatus(merged, { revoked: false });
}
