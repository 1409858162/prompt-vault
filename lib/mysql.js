...
function readConfig() {
  ...
  const cfg = {
    ...
    // Hard timeouts so a stuck TiDB pod can't pin a Vercel invocation forever.
    // TiDB serverless cold-starts can take 15-25s on the very first connect
    // after the cluster pauses; we budget for that here and let Vercel's
    // 30s maxDuration be the hard backstop.
    connectTimeout: parseInt(process.env.MYSQL_CONNECT_TIMEOUT_MS || '25000', 10),
  };
...
export async function transaction(fn) {
  ...
  while (attempt < 2) {
    ...
    try {
      ...
      if (attempt < 2 && isTransientConnError(err)) {
        await resetPool();
        continue;
      }
      throw err;
