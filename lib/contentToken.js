...
if (!SECRET) {
  throw new Error('CONTENT_TOKEN_SECRET or JWT_SECRET is required when NODE_ENV=production');
}
...
export function issueVideoKey({ userId, deviceId, resource }) {
  ...
  const key = crypto.createHmac('sha256', SECRET).update(seed).digest(); // 32 bytes
